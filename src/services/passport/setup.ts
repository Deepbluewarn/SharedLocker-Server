...
passport.use('login',
    new LocalStrategy({ usernameField: 'id' }, async (userId, password, done) => {
        UserService.findUserByObjectId(userId).then((user) => {
            if (!user) {
                return done(null, false, { success: false, message: '계정이 존재하지 않습니다.' });
            } else {
                AuthService.comparePassword(password, user.password).then((isMatch) => {
                    if (isMatch) {
                        ...
                        return done(null, false, { success: false, message: 'Invalid id or password.' });
                    }
                });
            }
        }).catch(err => {
    ...
    new LocalStrategy({ usernameField: 'id', passReqToCallback: true }, async (req, userId, password, done) => {
        ...
        UserService.findUserByObjectId(userId)
            .then(async (user) => {
                ...
                if (user) {
                    return done(null, false, { success: false, message: 'ID already exists.' });
                } else {
                    ...
                    UserService.saveNewUser(userId, hashedPwd)
                        .then((user) => {
                            ...
                            return done(null, user, { success: true, message: "성공적으로 가입되었습니다." });
                        });
                }
            })
            .catch((err) => {
...
passport.use('user',
    new JwtStrategy({
        ...
        UserService.findUserByObjectId(jwt_payload.id).then((user) => {
            if (user) {
                return done(null, user, { success: true, message: "유저 권한이 확인되었습니다." });
            } else {
                return done(null, false, { success: false, message: "유저를 찾을 수 없습니다." });
            }
        }).catch(err => {
            console.log('[passport user]: ', err);
            return done(err, false, {success: false, message: "예상치 못한 오류가 발생하였습니다."});
        });
    })
)
...
passport.use('token',
    new JwtStrategy({
        ...
        if (!jwt_payload) return done(null, false, {success: false, message: '토큰이 존재하지 않습니다.'});
